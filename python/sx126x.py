    ...
    def receive(self):
        if self.ser.inWaiting() > 0:
            ...
            if int(chr(r_buff[3])) == self.node_id or int(chr(r_buff[3])) == 0:
                print("Receive message from node address with frequence\033[1;32m %d,%d,%d.125MHz\033[0m"%((r_buff[0]<<8)+r_buff[0], r_buff[4],r_buff[2]+self.start_freq),end='\r\n',flush = True)

                print("Message is: "+str(r_buff[5:-1]),end='\r\n')
            else:
                print("%d sent a message but it was meant for: "%str(r_buff[4]),+ str(r_buff[3]))
