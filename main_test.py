...
def send_deal():
    ...
    get_rec = ""
    print("")
    print("input a string such as \032[1;32m0,0,868,Hello World\032[0m,it will send `Hello World` to lora node device of address 0 with node id 0 and 868M ")
    ...
    #         high 8bit address           low 8bit address           node id                  frequency                  address                  address                   node id            frequency                   message payload
    data = bytes([int(get_t[0])>>8]) + bytes([int(get_t[0])&0xff]) + bytes([int(get_t[1])]) + bytes([offset_frequence]) + bytes([node.addr>>8]) + bytes([node.addr&0xff]) + bytes([node.node_id]) + bytes([node.offset_freq]) + get_t[2].encode()
